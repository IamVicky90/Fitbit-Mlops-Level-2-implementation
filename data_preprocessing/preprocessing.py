...
class Preprocessor:
    ...
    def standardScalingData(self,X):
        ...
        self.save_model = self.file_op.save_model(scalar, 'StandardScaler')
    ...
    def impute_missing_values(self, data):
        ...
        self.data= data
        try:
            imputer=KNNImputer(n_neighbors=self.config['trainig_configurations']['KNNImputer']['n_neighbors'], weights=self.config['trainig_configurations']['KNNImputer']['weights'],missing_values=np.nan)
            self.new_array=imputer.fit_transform(self.data) # impute the missing values
            # convert the nd-array returned in the step above to a Dataframe
            self.new_data=pd.DataFrame(data=(self.new_array), columns=self.data.columns)
            self.file_op = file_methods.File_Operation(self.file_object,self.logger_object)
            self.save_model = self.file_op.save_model(KNNImputer, 'KNNImputer')
