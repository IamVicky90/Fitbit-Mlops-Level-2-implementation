...
class KMeansClustering:
    ...
    def elbow_plot(self,data):
        ...
        try:
            for i in range (1,self.config['trainig_configurations']['kmeans']['max_range_vale']):
                kmeans=KMeans(n_clusters=i,init='k-means++',random_state=self.config['trainig_configurations']['kmeans']['random_state']) # initializing the KMeans object
                kmeans.fit(data) # fitting the data to the KMeans Algorithm
                wcss.append(kmeans.inertia_)
            plt.plot(range(1,self.config['trainig_configurations']['kmeans']['max_range_vale']),wcss) # creating the graph between WCSS and the number of clusters
            plt.title('The Elbow Method')
            plt.xlabel('Number of clusters')
            plt.ylabel('WCSS')
            #plt.show()
            plt.savefig(self.config['trainig_configurations']['kmeans']['elbow_plot_path_to_save']) # saving the elbow plot locally
            # finding the value of the optimum cluster programmatically
            self.kn = KneeLocator(range(1, 11), wcss, curve=self.config['trainig_configurations']['kmeans']['KneeLocator']['curve'], direction=self.config['trainig_configurations']['kmeans']['KneeLocator']['direction'])
    ...
    def create_clusters(self,data,number_of_clusters):
        ...
        self.data=data
        try:
            self.kmeans = KMeans(n_clusters=number_of_clusters, init='k-means++', random_state=self.config['trainig_configurations']['kmeans']['random_state'])
